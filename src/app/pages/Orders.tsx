...
import { mockOrders, mockGTTOrders, mockHoldings, mockMargin, type Order, type GTTOrder } from "../data/mockData";
...
export default function Orders() {
  ...
  const [gttOrders, setGttOrders] = useState<GTTOrder[]>(mockGTTOrders);
  ...
  const heldQty = useMemo(() => {
    ...
    const h = mockHoldings.find((h) => h.symbol === orderForm.symbol);
    return h ? h.quantity : 0;
  }, [orderForm.symbol, orderForm.txType]);
  ...
  const marginWarning = useMemo(() => {
    ...
    if (orderValue > mockMargin.available * 0.2) {
      return `Order value (₹${orderValue.toLocaleString("en-IN")}) exceeds 20% of available margin (₹${(mockMargin.available * 0.2).toLocaleString("en-IN")}).`;
    }
    return null;
  }, [orderForm.quantity, orderForm.price, ltp]);
  ...
  const handleConfirmOrder = () => {
    const newOrder: Order = {
      ...
      triggerPrice: orderForm.triggerPrice ? Number(orderForm.triggerPrice) : undefined,
    };
    ...
    setOrders((prev) =>
      prev.map((o) =>
        o.id === modifyOrder.id
          ? {
              ...o,
              quantity: Number(modifyForm.quantity) || o.quantity,
              price: Number(modifyForm.price) || o.price,
              triggerPrice: modifyForm.triggerPrice ? Number(modifyForm.triggerPrice) : o.triggerPrice,
            }
          : o
      )
    );
  ...
  // US-073: GTT delete
  const handleDeleteGtt = (id: string) => {
            ...
            <div className="space-y-3">
              ...
              <Field label="Symbol">
                <select
                  ...
                  onChange={(e) => {
                    const h = mockHoldings.find((h) => h.symbol === e.target.value);
