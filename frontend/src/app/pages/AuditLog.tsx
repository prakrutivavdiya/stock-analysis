...
import { getAuditLogs, mapAuditLog } from "../api/audit";
import { ApiError } from "../api/client";

// M-05: Action type labels aligned with PRD naming
// TR-17: PAPER_TRADE added
...
export default function AuditLog() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const res = await getAuditLogs({
        ...
        tradingsymbol: symbolFilter.trim() || undefined,
        action_type: actionFilter !== "ALL" ? actionFilter : undefined,
        limit: 200,
        offset: 0,
      });
      setEntries(res.logs.map(mapAuditLog));
      setTotal(res.total);
    } catch (err: unknown) {
  ...
  }, [fromDate, toDate, symbolFilter, actionFilter]);
  ...
  const filtered = useMemo(() => {
    // Server already filters by action_type, date range, symbol.
    // Apply outcome filter and sorting client-side.
    let rows = [...entries];
    ...
    return rows;
  }, [entries, outcomeFilter, sortKey, sortDir]);
